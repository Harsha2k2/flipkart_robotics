...
if os.path.exists(model_weights_path):
    ...
    # Training loop with progress printing and loss tracking (only if no saved weights)
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Starting Epoch {epoch + 1}/{num_epochs}...")  # Indicate epoch start
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            # Print progress every few batches (e.g., every 10 batches)
            if (batch_idx + 1) % 10 == 0:  
...
# Save the model's state dictionary after training or loading existing weights
torch.save(model.state_dict(), model_weights_path)  
print("Model saved as 'fruit_freshness_model.pth'")

# Plotting the loss values if trained (optional)
if 'loss_values' in locals():  # Check if loss_values exists (only if trained)
